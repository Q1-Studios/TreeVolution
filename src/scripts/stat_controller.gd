...
func polen_heal(player, area, healing_array):
	while area.is_in_group("Polen"):
		print("s")
		pollen_healing(player, healing_array)
		#pollen_damage_buff(player, player.pollen_attributes["damageBuff"])
		await get_tree().create_timer(1).timeout

func pollen_healing(player, healing_array):
	var new_health
	if(healing_array[0]):
...
func pollen_damage_buff(player, damage_buff_array):
